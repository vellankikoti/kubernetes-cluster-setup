...
if ! kubectl get pods -A --field-selector=status.phase!=Running,status.phase!=Succeeded | awk 'NR>1 {print}' | grep -q .; then
  true
else
...
cat <<'YAML' | kubectl apply -n "$TEST_NS" -f - >/dev/null
apiVersion: v1
...
kubectl wait -n "$TEST_NS" --for=jsonpath='{.status.phase}'=Bound pvc/pvc-test --timeout=300s >/dev/null
...
kubectl rollout status -n "$TEST_NS" deployment/web --timeout=300s >/dev/null
kubectl expose deployment web -n "$TEST_NS" --port=80 --target-port=80 --type=ClusterIP --dry-run=client -o yaml | kubectl apply -f - >/dev/null
...
for _ in $(seq 1 32); do
  INGRESS_ADDR="$(kubectl -n ingress-nginx get svc ingress-nginx-controller -o jsonpath='{.status.loadBalancer.ingress[0].hostname}{.status.loadBalancer.ingress[0].ip}' 2>/dev/null || true)"
...
if [[ "$CLOUD" == "aws" ]]; then
  kubectl -n kube-system get deploy cluster-autoscaler >/dev/null
  ROLE_ARN="$(kubectl -n kube-system get sa cluster-autoscaler -o jsonpath='{.metadata.annotations.eks\.amazonaws\.com/role-arn}')"
  [[ -n "$ROLE_ARN" ]] || { echo "cluster-autoscaler serviceaccount missing IRSA annotation"; exit 1; }

  if kubectl -n kube-system logs deploy/cluster-autoscaler --tail=200 2>/dev/null | grep -Eqi 'AccessDenied|UnauthorizedOperation'; then
...
YAML
  ...
  kubectl scale deployment autoscaler-burst -n "$TEST_NS" --replicas=0 >/dev/null
...
kubectl delete ns "$TEST_NS" --wait=false >/dev/null 2>&1 || true
